...
/*Swipe-to-seek state*/
var isSeekingGesture = false;
var seekBaseTime = 0;
var seekOffset = 0;
var seekHUDEl = null;
var pxPerSecond = 8; // higher = less sensitive drag

function isVideoTouchTarget(e){
try{
return e.target.className.toString().includes("video-stream") || e.target.className.toString().includes("player-controls-background");
}catch(err){ return false; }
}

function showSeekHUD(offsetSeconds){
if(!seekHUDEl){
seekHUDEl=document.createElement("div");
seekHUDEl.setAttribute("id","ytproSeekHUD");
seekHUDEl.setAttribute("style",`
...
`);
document.body.appendChild(seekHUDEl);
}
var sign = offsetSeconds >= 0 ? "+" : "\u2212";
var abs = Math.round(Math.abs(offsetSeconds));
var arrows = offsetSeconds >= 0 ? "\u25B6\u25B6" : "\u25C0\u25C0";
seekHUDEl.innerHTML = `${arrows} ${sign}${abs}s`;
seekHUDEl.style.display="block";
}

function hideSeekHUD(){
if(seekHUDEl) seekHUDEl.style.display="none";
}
...
if(localStorage.getItem("saveCInfo") == null  || localStorage.getItem("gesC") == null || localStorage.getItem("gesM") == null || localStorage.getItem("bgplay") == null || localStorage.getItem("autoPip") == null){
...
<br>
<div>Auto Enter Picture-in-Picture <span data-action="sttCnf" data-value="autoPip" style="${sttCnf(0,0,"autoPip")}" ><b style="${sttCnf(0,1,"autoPip")}" ></b></span></div> 
<br>
<button data-action="openStudio">Open YouTube Studio</button>
<br>
<button data-action="musicMode">Toggle Music Mode</button>
<br>
...
var actionsList={
  ...
  openStudio:()=>{
    Android.openStudio();
    try{document.getElementById("settingsprodiv").remove();}catch(e){}
  },
  musicMode:()=>{
    toggleMusicMode();
    try{document.getElementById("settingsprodiv").remove();}catch(e){}
...
Android.setAutoPip(true);
}else{
Android.setAutoPip(false);
}
...
if (e.touches.length === 1 && isVideoTouchTarget(e)) {
var v = document.getElementsByClassName('video-stream')[0];
seekBaseTime = v ? v.currentTime : 0;
}
}, { capture: true });
...
/*Swipe left/right anywhere on the video (fullscreen or windowed) to seek*/
if (e.touches.length === 1 && localStorage.getItem("gesC") == "true" && isVideoTouchTarget(e)) {
var dx = e.touches[0].screenX - touchstartX;
var dy = e.touches[0].screenY - touchstartY;

if (isSeekingGesture || (Math.abs(dx) > 24 && Math.abs(dx) > Math.abs(dy) * 1.5)) {
var v = document.getElementsByClassName('video-stream')[0];
if (v && v.duration) {
isSeekingGesture = true;
stopProp = true;
seekOffset = dx / pxPerSecond;
var target = seekBaseTime + seekOffset;
if (target < 0) target = 0;
if (target > v.duration) target = v.duration;
seekOffset = target - seekBaseTime;
showSeekHUD(seekOffset);
}
}
}
},{capture:true});
...
var v = document.getElementsByClassName('video-stream')[0];
if (v) {
v.currentTime = seekBaseTime + seekOffset;
}
hideSeekHUD();
isSeekingGesture = false;
seekOffset = 0;
} else if((e.target.className.toString().includes("video-stream") || e.target.className.toString().includes("player-controls-background")) && !document.fullscreenElement && localStorage.getItem("gesM") == "true"){
checkDirection();
}
...
/*Tracks whether the player is currently in the mini (minimized) state*/
var isMinimized = false;

/*Called from Android's back-press handler. If a video is playing on a
watch/shorts page and it isn't minimized yet, minimize it (like the native
YouTube app) and tell Java to swallow the back press. Otherwise return
false so Java falls back to normal web navigation / closing the app.*/
function tryMinimizeOrBack(){
try{
var onWatch = window.location.href.indexOf("watch") > -1 || window.location.href.indexOf("shorts") > -1;
var v = document.getElementsByClassName("video-stream")[0];
if(onWatch && v && !v.paused && !isMinimized && !document.fullscreenElement){
minimize(true);
return true;
}
}catch(e){}
...
var miniW = rect.width * miniScale;
var miniH = rect.height * miniScale;
var margin = 10;
var bottomSafe = 70; // keep clear of the bottom nav bar

player.setAttribute("miniScale", miniScale);
player.style.position="fixed";
player.style.transformOrigin="top left";
player.style.transform="scale("+miniScale+")";
player.style.left=(window.innerWidth - miniW - margin)+"px";
player.style.top=(window.innerHeight - miniH - margin - bottomSafe)+"px";
player.style.zIndex="9999";
player.style.pointerEvents="auto";
player.style.cursor="grab";
player.style.borderRadius="10px";
player.style.overflow="hidden";
player.style.boxShadow="0 2px 12px rgba(0,0,0,0.6)";

/*Drag to reposition the mini player anywhere on screen*/
if(!player.getAttribute("ytproMiniDrag")){
player.setAttribute("ytproMiniDrag","true");

var dragging=false, dragMoved=false, startX=0, startY=0, startLeft=0, startTop=0;

player.addEventListener("touchstart", function(ev){
if(!isMinimized) return;
if(ev.target && (ev.target.id === "ytproMiniClose" || ev.target.id === "ytproMiniPlayPause")) return;
...
player.addEventListener("touchmove", function(ev){
...
var scale=parseFloat(player.getAttribute("miniScale"))||miniScale;
var w=player.getBoundingClientRect().width;
...
/*Tap (not drag) anywhere on the mini player to restore it*/
player.addEventListener("click", function ytproMiniTapHandler(ev){
if(!isMinimized) return;
...
closeBtn.setAttribute("id","ytproMiniClose");
closeBtn.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" height="14" viewBox="0 0 24 24" width="14"><path fill="#fff" d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>`;
...
ev.stopPropagation();
try{document.getElementsByClassName("video-stream")[0].pause();}catch(e){}
minimize(false);
});
...
/*Play/pause button*/
if(!document.getElementById("ytproMiniPlayPause")){
var ppBtn=document.createElement("div");
ppBtn.setAttribute("id","ytproMiniPlayPause");
ppBtn.setAttribute("style",`
position:absolute;bottom:4px;right:4px;z-index:10000;
width:22px;height:22px;border-radius:50%;background:rgba(0,0,0,0.65);
display:flex;align-items:center;justify-content:center;
`);
var updateIcon=()=>{
...
? `<svg xmlns="http://www.w3.org/2000/svg" height="14" viewBox="0 0 24 24" width="14"><path fill="#fff" d="M8 5v14l11-7z"/></svg>`
: `<svg xmlns="http://www.w3.org/2000/svg" height="14" viewBox="0 0 24 24" width="14"><path fill="#fff" d="M6 19h4V5H6zm8-14v14h4V5z"/></svg>`;
};
updateIcon();
ppBtn.addEventListener("click", function(ev){
ev.stopPropagation();
var v=document.getElementsByClassName("video-stream")[0];
if(!v) return;
...
player.removeAttribute("miniScale");

var closeBtn=document.getElementById("ytproMiniClose");
if(closeBtn) closeBtn.remove();
var ppBtn=document.getElementById("ytproMiniPlayPause");
if(ppBtn) ppBtn.remove();
...
/*Music Mode: replaces the visible video with a simple audio-style card.
Note: this does NOT reduce data usage - the same video stream keeps
decoding behind it. It's purely a distraction-free / battery-on-screen-dim
view. True bandwidth-saving audio-only playback would require swapping to
a separate audio-only stream, which is a much bigger follow-up feature.*/
var ytproMusicModeEl = null;
function toggleMusicMode(){
if(ytproMusicModeEl){
ytproMusicModeEl.remove();
ytproMusicModeEl = null;
return;
}
...
var titleEl = document.querySelector('h1.slim-video-metadata-title, yt-formatted-string.title, h1.title, .title.style-scope.ytd-video-primary-info-renderer');
var title = titleEl ? titleEl.textContent.trim() : "Playing audio";

var thumbLink = document.querySelector('link[rel="image_src"]');
var thumb = thumbLink ? thumbLink.href : "";

ytproMusicModeEl = document.createElement("div");
ytproMusicModeEl.setAttribute("id","ytproMusicMode");
ytproMusicModeEl.setAttribute("style", `
position:absolute;top:0;left:0;width:100%;height:100%;z-index:5000;
background:#000 center/cover no-repeat url('${thumb}');
display:flex;align-items:center;justify-content:center;
color:#fff;text-align:center;padding:16px;box-sizing:border-box;
`);
ytproMusicModeEl.innerHTML = `
<div style="background:rgba(0,0,0,0.55);padding:20px;border-radius:12px;max-width:90%;">
<div style="font-size:15px;font-weight:600;">${title}</div>
<div style="font-size:12px;opacity:0.7;margin-top:6px;">Audio-only view \u00b7 tap to exit</div>
</div>
`;
ytproMusicModeEl.addEventListener("click", toggleMusicMode);
player.appendChild(ytproMusicModeEl);
}
