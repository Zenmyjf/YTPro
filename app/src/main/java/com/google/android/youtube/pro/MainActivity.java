...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!prefs.contains("autoPip")) {
            prefs.edit().putBoolean("autoPip", true).apply();
        }
    ...
    public void load(boolean dl) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            // Skips the one-time Safe Browsing network check on first navigation.
            web.getSettings().setSafeBrowsingEnabled(false);
        }
        if (defaultUserAgent == null) {
            defaultUserAgent = web.getSettings().getUserAgentString();
        }
    ...
    private void handleBackPress() {
        // Ask the page whether a playing video should be minimized instead of
        // navigating back / closing the app, similar to the native YouTube app.
        web.evaluateJavascript("(function(){ try { return tryMinimizeOrBack(); } catch(e) { return false; } })()",
                value -> {
                    boolean minimized = "true".equals(value);
                    if (!minimized) {
                        runOnUiThread(() -> {
                            if (web.canGoBack()) {
                                web.goBack();
    ...
    protected void onUserLeaveHint() {
        ...
        SharedPreferences prefs = getSharedPreferences("YTPRO", MODE_PRIVATE);
        boolean autoPip = prefs.getBoolean("autoPip", true);
        if (autoPip && Build.VERSION.SDK_INT >= 26 && web.getUrl() != null && web.getUrl().contains("watch")) {
            if (isPlaying) {
                try {
                    isPip = true;
